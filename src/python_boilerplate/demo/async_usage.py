...
def non_coroutine() -> None:
    logger.warning("Should be raising TypeError after this log, cuz it's not an async function")


async def main() -> None:
    # Run both coroutines concurrently using asyncio.gather()
    results: list[Any] = await asyncio.gather(*[coroutine1(), coroutine2(), coroutine3()], return_exceptions=True)
