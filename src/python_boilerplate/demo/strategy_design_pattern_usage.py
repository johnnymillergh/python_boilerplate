...
class BaseStrategy:
    _strategies: Final[list["BaseStrategy"]] = []
    ...
    def init() -> None:
        ...
        for strategy_class in strategy_classes:
            BaseStrategy._strategies.append(strategy_class.__new__(strategy_class))
        logger.info(f"Initialized {len(strategy_classes)} subclasses: {BaseStrategy._strategies}")

    def strategy_name(self) -> str:
        return self.__class__.__name__

    def matches(self, data_input: str) -> bool:
        raise NotImplementedError(f"Function `{self.matches.__qualname__}` is not implemented")

    def execute(self, data_input: str) -> None:
        raise NotImplementedError(f"Function `{self.execute.__qualname__}` is not implemented")
    ...
    @classmethod
    def strategies(cls) -> list["BaseStrategy"]:
