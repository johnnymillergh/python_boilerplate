...
sony_published_video_games_path: Final = get_data_dir() / "sony_published_video_games.csv"
...
def look_for_sony_published_games() -> DataFrame:
    ...
    game_release_each_year: Final[Series] = (
        sony_published.groupby(release_year)[release_year].count().sort_values(ascending=False)
    )
...
def submit_parallel_tasks() -> list[DataFrame]:
    futures: list[Future[DataFrame]] = [generate_random_data(5000) for _ in range(5)]
    wait(futures)
...
def merge_results(dataframes: list[DataFrame]) -> DataFrame:
    result_list: DataFrame = pd.DataFrame(columns=["full_name", "age", "phone_number", "address", "zipcode", "country"])
...
def data_generation() -> None:
    ...
    logger.info(f"Done writing data file [{random_data_path}], rows: {len(result_data_pd)}")
