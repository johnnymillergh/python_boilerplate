...
async def test_non_coroutine() -> None:
    ...
    logger.info(f"Non-async function `non_coroutine()` raised exception: {exc_info.value}")
...
async def test_running_coroutine1_and_coroutine2_concurrently() -> None:
    ...
    logger.info(f"Type of `gathered_results`: {type(gathered_results)}, {gathered_results}")


@pytest.mark.asyncio
async def test_running_coroutine1_2_3_concurrently() -> None:
    gathered_results = await asyncio.gather(*[coroutine1(), coroutine2(), coroutine3()], return_exceptions=True)
    logger.info(f"Type of `gathered_results`: {type(gathered_results)}, {gathered_results}")
